...
function base(
  partial: Partial<AppendMessageInput> &
    Pick<AppendMessageInput, "messageId" | "kind" | "sender" | "eventRevision">,
): AppendMessageInput {
  return {
    session,
    priority: "normal",
...
describe("MessageRegistry — addressing & visibility (Req 1.1)", () => {
  it("delivers a direct message only to sender and recipient", () => {
    ...
    expect(reg.messagesFor(session, "alice").map((m) => m.messageId)).toEqual([
      "m1",
    ]);
    expect(reg.messagesFor(session, "bob").map((m) => m.messageId)).toEqual([
      "m1",
    ]);
  ...
  it("delivers a broadcast to everyone", () => {
    ...
    expect(reg.messagesFor(session, "carol").map((m) => m.messageId)).toEqual([
      "m1",
    ]);
  });
});
...
describe("MessageRegistry — questions & answers (Req 1.3)", () => {
  it("marks a question answered when a correlated answer arrives", () => {
    ...
    expect(
      reg.openQuestionsFor(session, "bob").map((m) => m.messageId),
    ).toEqual(["q1"]);

    const answer = reg.append(
      base({
        messageId: "a1",
        kind: "answer",
        sender: bob,
...
describe("MessageRegistry — ordering & restore", () => {
  it("keeps messages ordered by eventRevision regardless of append order", () => {
    ...
    expect(reg.allMessages(session).map((m) => m.eventRevision)).toEqual([
      1, 2, 3,
    ]);
  });
